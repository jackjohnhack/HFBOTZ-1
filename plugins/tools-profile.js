...
let handler = async (m, { conn, text, usedPrefix, command }) => {
 ...
 let pp = 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSXIdvC1Q4WL7_zA6cJm3yileyBT2OsWhBb9Q&usqp=CAU'
  try {
    ...
    let { name, pasangan, limit, exp, bank, lastclaim, premiumTime, premium, registered, regTime, age, level, role } = global.db.data.users[who]
    let now = new Date() * 1
...
Kadaluarsa Premium: ${(premiumTime - now) > 1 ? msToDate(premiumTime - now) : '*Tidak diatur expired premium!*'}${lastclaim > 0 ? '\nLast Claim: ' + new Date(lastclaim) : ''}
`.trim()
